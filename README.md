...
* **[phase 2]嵌入式工程（Embedded Engineering）**
  ...
  * 当前物理蓝图 (MVP 1.0)
  *视觉介质：基于 LCD 屏幕驱动的倒置透明金字塔（佩珀尔幻象光学原理）。

  * 动力执行：使用 SG90 舵机实现物理转向（偏航轴/Yaw）。环境反馈：集成 WS2812B LED 灯环，用于情绪光影映射。

* **已知局限与瓶颈**
  
  * 通过理论建模，我识别出 1.0 设计中存在几个关键的 UX 与物理限制：

  * 光学视角（45° 约束）：当用户从俯视角度观察时，佩珀尔幻象会失效，这限制了用户的自然人体工学姿势（迫使用户必须俯身平视）。

  * 自由度受限 (DOF)：单舵机设置缺乏俯仰轴（Pitch），导致代理无法与不同身高的用户主动保持“眼神交流”。

* **体积感差距：与新兴的光场技术相比，平面投影仍缺乏真正的空间深度。**
---
